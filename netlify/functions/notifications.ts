...
type EmailTemplateComponent = (notificationId: number, payload: Prisma.JsonValue) => ReactElement;
...
async function sendNotificationEmail(notification: NotificationToSend) {
  ...
  try {
    const html = await render(TemplateComponent(notification.id, notification.payload));
