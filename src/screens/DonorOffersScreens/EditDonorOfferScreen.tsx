...
export default function EditDonorOfferScreen() {
  ...
  useEffect(() => {
    (async () => {
      const response = await fetch(`/api/donorOffers/${donorOfferId}/edit`);
      const {
        offerName,
        donorName,
      ...
      setPartnerRequestDeadline(
        new Date(partnerResponseDeadline).toISOString().substring(0, 10)
      );
      setDonorRequestDeadline(
        new Date(donorResponseDeadline).toISOString().substring(0, 10)
      );
      setSelectedPartners(
        partnerVisibilities.map((v: { partner: Partner }) => v.partner)
      );
      setData(items);
    })();
  }, [donorOfferId]);
