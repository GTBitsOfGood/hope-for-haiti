...
export default function PartnerWishlistScreen({ partnerId, readOnly = false }: { partnerId?: number; readOnly?: boolean }) {
  ...
  const [originals, setOriginals] = useState<Map<number, WishlistItem>>(new Map());
  ...
  return (
    <div className="pb-32">
      <CommentModal
        ...
        initialValue={activeCommentId ? ((items.find(i => i.id === activeCommentId) as WishlistItem & { comments?: string; comment?: string })?.comments ?? (items.find(i => i.id === activeCommentId) as WishlistItem & { comments?: string; comment?: string })?.comment ?? "") : ""}
        onClose={() => {
      ...
      <BaseTable
        ...
        rows={items.map((it) => {
          const lastUpdated = (it as WishlistItem & { updatedAt?: Date }).updatedAt ? new Date((it as WishlistItem & { updatedAt?: Date }).updatedAt!).toLocaleDateString() : new Date().toLocaleDateString();
          ...
          return {
            cells: isEditingItem ? [
              ...
              <PriorityTag key={`delete-priority-${it.id}`} priority={it.priority} />,
              lastUpdated,
              <div key={`delete-comment-${it.id}`}>
                <ChatTeardropText
                  data-tooltip-id={`wishlist-comment-${it.id}`}
                  data-tooltip-content={(it as WishlistItem & { comments?: string; comment?: string }).comments || (it as WishlistItem & { comments?: string; comment?: string }).comment || ""}
                  size={22}
                ...
                />
                {((it as WishlistItem & { comments?: string; comment?: string }).comments || (it as WishlistItem & { comments?: string; comment?: string }).comment) && (
                  <Tooltip key={`wishlist-comment-${it.id}-${(it as WishlistItem & { comments?: string; comment?: string }).comments || (it as WishlistItem & { comments?: string; comment?: string }).comment}`} id={`wishlist-comment-${it.id}`} className="max-w-64 whitespace-pre-wrap" />
                )}
              </div>,
              ...
              <PriorityTag key={`view-priority-${it.id}`} priority={it.priority} />,
              lastUpdated,
